...
export function getAdornmentStyleAdjustmentForNativeInput({
    ...
    if (adornmentConfig.length) {
        const adornmentStyleAdjustmentForNativeInput = adornmentConfig.map(({ type, side }: AdornmentConfig) => {
            const isLeftSide = side === AdornmentSide.Left;
            const inputModeAdornemntOffset =
                mode === InputMode.Outlined ? ADORNMENT_OFFSET + OUTLINED_INPUT_OFFSET : ADORNMENT_OFFSET;
...
const captalize = (text: string) => text.charAt(0).toUpperCase() + text.slice(1);
