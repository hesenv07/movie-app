...
type Props = Omit<React.ComponentProps<typeof IconButton>, 'icon' | 'theme' | 'color'> & {
    /**
     * Icon to show.
     */
    name: IconSource;
...
> = ({ icon, topPosition, side, isTextInputFocused, forceFocus }) => {
    ...
    return <StyleContext.Provider value={contextState}>{icon}</StyleContext.Provider>;
};
...
const TextInputIcon = ({ name, onPress, forceTextInputFocus, color, ...rest }: Props) => {
    const { style, isTextInputFocused, forceFocus } = React.useContext(StyleContext);
